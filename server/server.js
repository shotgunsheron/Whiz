...
io.on('connection', (socket) => {
    ...
    socket.on('host-join', (data) =>{
        ...
        MongoClient.connect(url, function(err, db) {
            if (err) throw err;
            var dbo = db.db("whizDB");
            var query = { id:  parseInt(data.id)};
            dbo.collection('whizGames').find(query).toArray(function(err, result){
                if(err) throw err;
    ...
    socket.on('host-join-game', (data) => {
        ...
        if(game){
            ...
            MongoClient.connect(url, function(err, db){
                ...
                dbo.collection("whizGames").find(query).toArray(function(err, res) {
                    if (err) throw err;
    ...
    socket.on('requestDbNames', function(){
        ...
        MongoClient.connect(url, function(err, db){
            ...
            var dbo = db.db('whizDB');
            dbo.collection("whizGames").find().toArray(function(err, res) {
                if (err) throw err;
    ...
    socket.on('newQuiz', function(data){
        MongoClient.connect(url, function(err, db){
            if (err) throw err;
            var dbo = db.db('whizDB');
            dbo.collection('whizGames').find({}).toArray(function(err, result){
                if(err) throw err;
                ...
                var game = data;
                dbo.collection("whizGames").insertOne(game, function(err, res) {
                    if (err) throw err;
                    db.close();
                });
                db.close();
